...
export default function App() {
  ...
  const timer = setTimeout(async () => {
    if (!window.html2pdf || !pdfRef.current || autoGenerated) return;

    const element = pdfRef.current;

    try {
      // Step 1: Render to canvas manually first
      ...
      // Step 2: Compute the real height in mm
      const pxHeight = canvas.height;
      const mmHeight = (pxHeight * 25.4) / 96; // 96dpi default
      const pdfHeight = Math.max(mmHeight + 10, 120); // always >=120mm

      // Step 3: Build pdf manually (avoids cropping)
      ...
      pdf.addImage(imgData, "JPEG", 0, 0, 80, pdfHeight);
      pdf.save(`advisory_receipt_${receipt_id}.pdf`);
...
const waitForFonts = async () => {
  ...
  color: "#000",
    ...
    telugu: { fontFamily: "'Noto Sans Telugu', Lexend, sans-serif",
  fontSize: "9px", },
  };
